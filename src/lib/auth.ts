...
// @ts-expect-error - next-auth/jwt module resolution workaround for Next 16 + bundler moduleResolution
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (session.user) {
        const t = token as { id?: string; roleId?: string; employeeId?: string };

    ...
    <main className="relative flex min-h-screen overflow-hidden bg-background px-6 py-8">
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-6xl items-center gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="hidden min-h-[620px] flex-col justify-between rounded-[2rem] border border-white/70 bg-white/55 p-8 shadow-2xl shadow-blue-950/10 backdrop-blur-md lg:flex">
          <div>
            ...
            <p className="mt-5 max-w-lg text-sm leading-6 text-muted-foreground">
              Login menggunakan akun kerja Anda untuk membuka modul HRD yang relevan.
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              <article key={item.label} className="rounded-2xl border border-white/70 bg-white/70 p-4 shadow-sm">
                <span className="text-xl">{item.icon}</span>
                <strong className="mt-3 block text-2xl text-foreground">{item.value}</strong>
        ...
        <div className="mx-auto w-full max-w-md rounded-[2rem] border border-border bg-card p-6 shadow-2xl shadow-blue-950/10 sm:p-8">
          <div className="mb-8 flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 text-primary">
                <Sparkles size={18} />
                <span className="text-xs font-bold uppercase tracking-[0.18em]">Secure Login</span>
              </div>
              <h2 className="mt-3 text-3xl font-extrabold text-foreground">Masuk ke HRD</h2>
          ...
          {error && (
            <div className="mb-4 rounded-2xl bg-rose-50 border border-rose-200 p-4 text-sm font-medium text-rose-700">
              {error}
            </div>
          )}
...
>
            ...
            <div className="flex items-center justify-between gap-3 text-xs">
              <label className="flex items-center gap-2 font-medium text-muted-foreground">
                <input name="remember" type="checkbox" className="size-4 rounded border-input accent-primary" />
                Ingat saya
              </label>
            ...
            <Button type="submit" size="lg" className="h-12 w-full rounded-2xl text-sm font-bold" disabled={loading}>
